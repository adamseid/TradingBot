...
def getDPriceStdev():
    now = int(time.time())
    then = now - 15*60  # 3600 seconds = 1 hour
    queryset = MasterDB.objects.filter(unix__range=(str(then), str(now)))
    data = list(queryset.values('dprice'))
    df = pd.DataFrame(data)
    # filter out rows with empty values
    try:
        df = df[df['dprice'] != '']
        df['dprice'] = df['dprice'].astype(float)
        std_deviation = df['dprice'].std()
    except:
        std_deviation = 0
    return std_deviation
...
def getDPrice2Stdev():
    now = int(time.time())
    then = now - 5*60  # 3600 seconds = 1 hour
    queryset = MasterDB.objects.filter(unix__range=(str(then), str(now)))
    data = list(queryset.values('dprice'))
    df = pd.DataFrame(data)
    # filter out rows with empty values
    try:
        ...
        std_deviation *= 2
        # Filter out values outside of the two standard deviations
        df = df[(df['dprice'] >= mean - std_deviation) &
                (df['dprice'] <= mean + std_deviation)]
        std_deviation = df['dprice'].std()
    except:
        std_deviation = 0
    return std_deviation
